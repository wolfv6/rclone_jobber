...
To setup logrotate, setup /var/log/rclone_jobber.log (described in previous section).
...
All you have to do is setup /var/log/rclone_jobber.log (described in section "# set log_file path to /var/log/rclone_jobber.log") and make these two changes to rclone_jobber.sh script:
...
Here is how to setup the recovery-plan backup for easy access.
...
That way the recovery-plan files are with the backups, and accessible before installing rclone.
...
2. make sure the recovery-plan files are still accessible and up-to date (listed in "[[*Recovery plan][Recovery plan]]" section)
   - on site copy
   - off site copy
