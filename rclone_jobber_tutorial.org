...
* Logging options
** Log output
Look for "set" in rclone_jobber.sh, for example:
#+BEGIN_EXAMPLE
...
In this example, change "wolfv" to your user name:
#+BEGIN_EXAMPLE
...
In rclone_jobber.sh, change log_file value to /var/log/rclone_jobber.log
...
Linux and macOS can send all log output to systemd journal by making the following changes to rclone_jobber.sh script.

Find lines containing "log_file" and change all appends to log_file from something that looks like this:
#+BEGIN_EXAMPLE
    echo "$message" >> "$log_file"
#+END_EXAMPLE
to this:
#+BEGIN_EXAMPLE
   printf "$message" | systemd-cat -t RCLONE_JOBBER -p info
#+END_EXAMPLE

In cmd="rclone sync ..", change this option:
#+BEGIN_EXAMPLE
    --log-file=$log_file
#+END_EXAMPLE
to this:
